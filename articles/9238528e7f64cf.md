...
内容としては、Goで実際に現場でアプリケーションを構築している方向けかもしれません。初学者の方には優しくない内容かもしれませんが、ご了承ください。
※ ORMにはgormを使用しています。
...
update(user)
```
となると思います。ただ、これだと、複数テーブルを更新する際に、どうやってアトミック性を担保するのかがずっと疑問でした。repository層はテーブルと1対1で対応させていたので、テーブルAからデータ取得、テーブルBでデータ取得、Aを更新、Bを更新時にエラー、となると当然ロールバックする必要があります。その際の書き方が明確になっていないことが多く、「どうするんだろう？」と思っていました。
...
type Transaction interface {
    // BeginTransaction()の後、Commit()またはRollback()を呼びだす
    Commit() error 
    Rollback()
    GetTx() *gorm.DB
}
...
func (t *transaction) GetTx() Transaction { return t.tx }
func (t *transaction) Commit() error { return
t.tx.Commit().Error }
...
## Txは最後の引数にする
一時は、txを引数の最後に必ず渡すようにし、dbの実行直前にnilかどうかをチェックして
実行するという方法も試しました。明瞭さと引き換えにメソッドの数を減らせたものの、先ほどと比べて特段メリットがあるわけではありません。
それにこれだと今度は、txを渡し忘れたりするリスクが出てきます。
...
tx.Rollback等を気にせず、ビジネスロジックの実装に集中できるようになりました。おすすめの手法です。
何か開発のヒントになれば幸いです。
